...
export function createMultiValidatorForMultiData(validators) {

    const multiValidator = createMultiValidator(validators);

    function validate(multiData=[], dataBailOnFail=false, validatorBailOnFail=false) {
        ...
        for(let i = 0; i < multiDataLength; i++) {
            ...
            const result = multiValidator.validate(data, validatorBailOnFail);
